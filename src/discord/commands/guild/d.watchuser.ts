...
export const dWatchUser: SlashCommand = {
  ...
  async execute(interaction) {
    ...
    if (await executeWatch(target, notificationMethod, interaction.user.id, alertChannel)) {
      ...
      await interaction.editReply({ content: 'Whoops, it seems like you\'re already watching this user! If you\'d like, you can cancel this or cancel and switch modes, by using /watch cancel.' }); // eslint-disable-line max-len
    }
